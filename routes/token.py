...
from oauth2.jwt import ACCESS_TOKEN_EXPIRE_MINUTES, ALGORITHM, SECRET_KEY, Token, TokenData ,oauth2_scheme
...
async def login_for_access_token(form_oauth : OAuth2PasswordRequestForm = Depends()):
    login_user = authenticate_user(Fake_users_db, form_oauth.username, form_oauth.password)
...
@token.get("/api/v1/token/me/", response_model=User,  tags=["Token"], description="Get data user validation")
